...
subplot 321; plot(x,'b'); axis tight;
title('(a)元音/a/波形'); ylabel('幅值')
subplot 322; plot(y,'b'); xlim([0 L]); 
title('(b)一帧数据'); ylabel('幅值')
subplot 323; plot(est_x1,'b'); xlim([0 L]); 
title('(c)LPC预测值'); ylabel('幅值')
subplot 324; plot(est_x2,'b'); xlim([0 L]); 
title('(d)lpc\_coeff预测值'); ylabel('幅值')
subplot 325; plot(err1,'b'); xlim([0 L]); 
title('(e)LPC预测误差'); ylabel('幅值'); xlabel('样点')
subplot 326; plot(err2,'b'); xlim([0 L]); 
